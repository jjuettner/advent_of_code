...
for game in input:
  game_possible = True
  game_id = game.split(":")[0].split(" ")[1]      # 1
  round = game.split(":")[1].split(";")           # 1 green, 8 blue, 4 red
  maxResults = {"red": 0,"green": 0,"blue": 0}
  for roll in round:
    results = roll.split(",")                     # 1 green
    for result in results:
      color = result.strip().split(" ")[1]        # green
      amount = int(result.strip().split(" ")[0])  # 1
